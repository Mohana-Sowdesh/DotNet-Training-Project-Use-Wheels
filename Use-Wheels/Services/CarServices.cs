...
namespace Use_Wheels.Services
{
	public class CarServices : ICarServices
	{
        ...
        public async Task<Car> AddCar(CarDTO carDTO, string username, string jwtToken)
        {
            ...
            // Checks if date of registration is before today
            DateTime dateOnly = new DateTime(carDTO.Rc_Details.Date_Of_Reg.Year, carDTO.Rc_Details.Date_Of_Reg.Month, carDTO.Rc_Details.Date_Of_Reg.Day);
            if (dateOnly.CompareTo(DateTime.Today.Date) > 0)
                throw new BadHttpRequestException(Constants.CarConstants.DATE_OF_REG_ERROR, Constants.ResponseConstants.BAD_REQUEST); ;
        ...
        public async Task<IEnumerable<CarResponseDTO>> GetAllCars(string role)
        {
            ...
            List<VehicleInfoDTO> blackedCars = await _apiUtility.GetMockDataAsync();
            IEnumerable<CarResponseDTO> carDTOList = _mapper.Map<IEnumerable<CarResponseDTO>>(carList);

            foreach (CarResponseDTO car in carDTOList)
            {
        ...
        /// <param name="vehicle_no"></param>
        /// <returns>Car object</returns>
        public async Task<CarResponseDTO> GetCarById(string vehicle_no, string role)
        {
            int validationResult = adminCarUtility.isVehicleNoValid(vehicle_no);
            ...
            CarResponseDTO carDTO = _mapper.Map<CarResponseDTO>(car);
            if (mockApiDataCheckResult == 2)
